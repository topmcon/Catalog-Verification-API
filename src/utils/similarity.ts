 ...
 */
export function compareValues(value1: unknown, value2: unknown): number {
  ...
  if (typeof value1 === 'object' && typeof value2 === 'object') {
    return compareObjects(value1 as Record<string, unknown>, value2 as Record<string, unknown>);
  }
 ...
 * Compare two arrays
 */
export function compareArrays(arr1: unknown[], arr2: unknown[]): number {
 ...
 */
export function compareAIResults(
  openaiResult: AIValidationResult,
  xaiResult: AIValidationResult
): AIComparisonMetrics {
  logger.debug('Comparing AI validation results');
  ...
  if (commonFields.length > 0) {
    for (const field of commonFields) {
      valueSimilarity += compareValues(
        openaiResult.verifiedFields[field],
        xaiResult.verifiedFields[field]
      );
    }
  ...
  const metrics: AIComparisonMetrics = {
  ...
  logger.debug('AI comparison metrics', metrics);
  return metrics;
}
...
export function identifyDiscrepancies(
  ...
  const discrepancies: ConsensusDiscrepancy[] = [];
  ...
  logger.info(`Found ${discrepancies.length} discrepancies between AI results`);
