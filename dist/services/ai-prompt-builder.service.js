 ...
 */
function buildVerificationPrompt(rawProduct, categorySchema, _provider, retryContext) {
...
${category_schema_1.GLOBAL_PRIMARY_ATTRIBUTES.map((attr, i) => `${i + 1}. ${attr}`).join('\n')}
...
6. **Missing Data**: If a required field cannot be found in the raw data, set to null and flag for research.
...
function buildRetryInstructions(context) {
 ...
 */
function determineBrandTier(brand) {
    if (!brand)
        return '';
    const normalizedBrand = brand.toUpperCase();
    if (category_schema_1.PREMIUM_BRANDS.some(b => normalizedBrand.includes(b.toUpperCase()))) {
        return '- Brand Tier: PREMIUM/LUXURY (High-end pricing expected)';
    }
    if (category_schema_1.MID_TIER_BRANDS.some(b => normalizedBrand.includes(b.toUpperCase()))) {
 ...
 */
function formatAttributeList(attrs) {
 ...
 */
function buildResearchPrompt(modelNumber, brand, category, missingFields) {
...
//# sourceMappingURL=ai-prompt-builder.service.js.map