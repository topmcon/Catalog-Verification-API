...
function getClient() {
    if (!xaiClient) {
        if (!config_1.default.xai.apiKey) {
            throw new Error('xAI API key is not configured');
        }
        xaiClient = axios_1.default.create({
            baseURL: config_1.default.xai.apiUrl,
            headers: {
                'Authorization': `Bearer ${config_1.default.xai.apiKey}`,
...
async function validateProduct(request, retryContext) {
    ...
    logger_1.default.info(`xAI validation started for product ${product.originalId}`, { sessionId });
    try {
        const client = getClient();
        const prompt = buildValidationPrompt(product, retryContext);
        const response = await client.post('/chat/completions', {
            model: config_1.default.xai.model,
            messages: [
                {
                    role: 'system',
        ...
        logger_1.default.debug(`xAI raw response for ${product.originalId}`, { rawResponse });
        ...
        try {
            ...
            logger_1.default.error(`Failed to parse xAI response for ${product.originalId}`, { rawResponse });
        ...
        logger_1.default.info(`xAI validation completed for product ${product.originalId}`, {
            sessionId,
        ...
        logger_1.default.error(`xAI validation failed for product ${product.originalId}`, {
            sessionId,
            error: errorMessage,
        });
        return {
...
async function validateProducts(products, sessionId) {
    logger_1.default.info(`xAI batch validation started for ${products.length} products`, { sessionId });
    const results = [];
    ...
    logger_1.default.info(`xAI batch validation completed for ${products.length} products`, { sessionId });
    return results;
}
/**
...
//# sourceMappingURL=xai.service.js.map